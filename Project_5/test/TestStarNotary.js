...
it('can add the star name and star symbol properly', async() => {
    ...
    await instance.setName('Hello World');
    await instance.setSymbol('HW');
    assert.equal(await instance.name(), "Hello World");
    assert.equal(await instance.symbol(), "HW");
});
...
it('lets 2 users exchange stars', async() => {
    ...
    assert.equal(accounts[1], await instance.owner(starId2));
    assert.equal(accounts[2], await instance.owner(starId1));
});
...
it('lets a user transfer a star', async() => {
    ...
    assert.equal(accounts[1], await instance.owner(tokenId));
});
